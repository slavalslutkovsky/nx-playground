...
impl SmtpProvider {
    ...
    ///
    /// # Note
    /// This method is simpler but less secure than OAuth2. Best for development/testing.
    ...
    ///    - `EMAIL_FROM_NAME` (optional)
    ///
    /// # Setup (SMTP AUTH with OAuth2)
    /// For OAuth2 authentication, use the `GmailOAuth2Provider` instead.
    ///
    /// # Note
    /// - Only works with Google Workspace (not personal Gmail)
    /// - IP allowlisting is configured in Google Admin Console
    /// - No authentication needed if IP is allowlisted
    ...
    ///
    /// # Setup
    /// Configure SMTP relay in Google Admin Console to require SMTP AUTH.
    /// Set environment variables:
    /// - `GMAIL_RELAY_USER` - Workspace user email
    /// - `GMAIL_RELAY_PASSWORD` - User password or app password
    /// - `EMAIL_FROM_ADDRESS` - Must be from your Workspace domain
