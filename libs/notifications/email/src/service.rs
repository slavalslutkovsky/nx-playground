...
//!
//! This service provides a high-level API for queueing emails.
//! It uses `EmailStream` constants for stream names to ensure
...
impl Default for NotificationServiceConfig {
    fn default() -> Self {
        Self {
            max_stream_length: EmailStream::MAX_LENGTH,
...
///
/// Uses `EmailStream` constants for stream names to ensure
/// the same names are used by both producer and consumer.
...
impl NotificationService {
    ...
    /// Get the stream name (from EmailStream constants).
    pub fn stream_name(&self) -> &'static str {
        EmailStream::STREAM_NAME
    }

    /// Get the consumer group name (from EmailStream constants).
    ...
    pub async fn ensure_consumer_group(&self) -> NotificationResult<()> {
        ...
        let result: Result<(), redis::RedisError> = redis::cmd("XGROUP")
            .arg("CREATE")
            .arg(EmailStream::STREAM_NAME)
            .arg(EmailStream::CONSUMER_GROUP)
            .arg("$")
            .arg("MKSTREAM")
            .query_async(&mut conn)
            .await;
        ...
        match result {
            Ok(_) => {
                info!(
                    stream = %EmailStream::STREAM_NAME,
                    group = %EmailStream::CONSUMER_GROUP,
    ...
    pub async fn get_pending_count(&self) -> NotificationResult<i64> {
        let mut conn = (*self.redis).clone();
        let len: i64 = conn.xlen(EmailStream::STREAM_NAME).await?;
        Ok(len)
    }
}
...
mod tests {
    ...
    fn test_default_config() {
        ...
        assert_eq!(config.max_stream_length, EmailStream::MAX_LENGTH);
