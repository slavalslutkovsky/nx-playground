...
/// Retry configuration for gRPC channel connections
///
/// Mirrors the retry configuration from the database library for consistency.
#[derive(Debug, Clone)]
...
impl RetryConfig {
    ...
    /// # Example
    /// ```ignore
    /// let config = RetryConfig::new().with_initial_delay(200);
    /// ```
    ...
    /// # Example
    /// ```ignore
    /// let config = RetryConfig::new().with_max_delay(10000);
    /// ```
...
///
/// This function will retry a failed operation up to `max_retries` times,
/// with exponentially increasing delays between attempts. Useful for
/// establishing connections to services that may not be immediately available.
///
/// # Arguments
...
/// # Example
/// ```ignore
/// use grpc_client::{retry_with_backoff, RetryConfig};
///
/// let config = RetryConfig::new().with_max_retries(5);
///
/// let channel = retry_with_backoff(
///     || async { create_channel("http://service:50051").await },
///     config
/// ).await?;
/// ```
            ...
            Ok(result) => {
                if attempt > 0 {
                    debug!(
                        target: "grpc_client",
                        attempts = attempt,
                        "gRPC operation succeeded after retries"
                    );
                }
                return Ok(result);
            }
            Err(e) => {
                ...
                if attempt > config.max_retries {
                    warn!(
                        ...
                        error = %e,
                        "gRPC operation failed after max retries"
                    );
                    return Err(e);
                }
...
/// Equivalent to calling `retry_with_backoff` with `RetryConfig::default()`.
///
/// # Example
/// ```ignore
/// use grpc_client::retry;
///
/// let channel = retry(|| async {
///     create_channel("http://service:50051").await
/// }).await?;
/// ```
