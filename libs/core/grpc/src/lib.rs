...
pub use channel::{create_channel, create_channel_with_config, create_channel_with_retry, ChannelConfig};
pub use client::{
    configure_client, with_compression, with_zstd_compression,
    with_limits, with_standard_limits, ConfigurableClient,
};
pub use error::{GrpcError, GrpcResult, ToTonicResult, ToTonicOption};
pub use retry::{RetryConfig, retry, retry_with_backoff};

// Re-export interceptors for convenience
pub use interceptors::{
    AuthInterceptor, TracingInterceptor, MetricsInterceptor,
    compose_interceptors, ComposedInterceptor,
};
