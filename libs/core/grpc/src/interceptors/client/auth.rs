...
//!
//! Injects authentication headers into outgoing gRPC requests.
...
/// # Example
/// ```ignore
/// use grpc_client::interceptors::client::AuthInterceptor;
...
impl AuthInterceptor {
    ...
    /// Create an interceptor with an API key in the authorization header
    ///
    /// # Example
    /// ```ignore
    ...
    /// Create an interceptor with an API key in a custom header
    ///
    /// # Example
    /// ```ignore
    /// let auth = AuthInterceptor::api_key_header("x-api-key", "my-api-key-12345");
    /// ```
    pub fn api_key_header(header_name: &'static str, key: impl Into<String>) -> Self {
        Self {
            header_name,
            header_value: key.into(),
        }
    ...
    /// Create a dynamic auth interceptor that fetches token on each request
    ///
    /// Useful for tokens that expire and need to be refreshed.
    ///
    /// # Example
    /// ```ignore
    /// let auth = AuthInterceptor::dynamic(|| async {
    ///     get_current_token().await
    /// });
    /// ```
    pub fn with_header(header_name: &'static str, value: impl Into<String>) -> Self {
        Self {
            header_name,
            header_value: value.into(),
        }
...
mod tests {
    ...
    fn test_custom_header() {
        let mut auth = AuthInterceptor::api_key_header("x-api-key", "secret-key");
        let request = Request::new(());
        let result = auth.call(request);
        assert!(result.is_ok());
        let req = result.unwrap();
        let header = req.metadata().get("x-api-key").unwrap();
        assert_eq!(header, "secret-key");
    }
