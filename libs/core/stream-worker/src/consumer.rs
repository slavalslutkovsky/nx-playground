...
impl StreamConsumer {
    ...
    pub async fn read_pending<J: StreamJob + DeserializeOwned>(
        ...
        let result: RedisResult<StreamReadResult> = redis::cmd("XREADGROUP")
            .arg("GROUP")
            .arg(&self.config.consumer_group)
    ...
    pub async fn read_new<J: StreamJob + DeserializeOwned>(
        ...
        let result: RedisResult<Option<StreamReadResult>> = cmd.query_async(&mut conn).await;
    ...
    pub async fn claim_abandoned<J: StreamJob + DeserializeOwned>(
        ...
        let result: RedisResult<StreamEntries> = cmd.query_async(&mut conn).await;
